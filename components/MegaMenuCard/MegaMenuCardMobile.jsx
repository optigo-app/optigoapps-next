...
const SectionLink = ({ item, setActiveMenu, setDrawerOpen }) => {
...
const AccordionSection = ({ title, items, linkText, setActiveMenu, setDrawerOpen }) => {
    const handleNavigation = useHandleUrlNavigation();
    return (
        <AccordionItem value={title}>
            ...
            <AccordionContent>
                <ul className="space-y-2 pl-4">
                    {items.map((item, idx) => (
                        <SectionLink key={idx} item={item} setActiveMenu={setActiveMenu} setDrawerOpen={setDrawerOpen} />
                    ))}
                    {linkText && (
    ...
    return (
        <div className="absolute w-full left-0 bg-[#f7f7f8] shadow-2xl rounded-md p-6 overflow-y-auto max-h-[90vh] animate-in">
            <Accordion type="multiple" className="space-y-4">
                <AccordionSection
                    title={title === "About Us" ? "Know More" : "Industries"}
